...
export default function Dashboard() {
  ...
  // ★ フィルター変更時に、選択中ライバーが対象外になったらCRMを閉じる
  useEffect(() => {
    if (selectedLiverId) {
    ...
    <div className="min-h-screen bg-[#020617] text-slate-50 p-4 md:p-8 font-sans selection:bg-indigo-500/30 overflow-x-hidden">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <header className="flex flex-col xl:flex-row xl:items-center justify-between gap-6 border-b border-slate-800/80 pb-6">
          <div><h1 className="text-3xl font-black tracking-tight bg-gradient-to-r from-indigo-400 to-cyan-400 bg-clip-text text-transparent">Hiyoko Intelligence</h1><p className="text-slate-400 mt-1 text-sm font-medium">TikTok Live Strategic Management</p></div>
          <div className="flex flex-col md:flex-row gap-4 items-start md:items-center">
            ...
            {activeTab === 'custom' && (
              <div className="flex items-center space-x-2 bg-slate-900/80 p-1.5 rounded-xl border border-slate-800 shadow-inner animate-in fade-in">
                ...
                <button onClick={handleCustomFetch} disabled={!startDate || !endDate} className="bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white px-4 py-2 rounded-lg text-xs font-bold transition-colors">解析</button>
              </div>
            )}
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-slate-900/60 border border-slate-800/80 p-6 rounded-3xl shadow-sm flex flex-col justify-center backdrop-blur-sm">
            <div className="flex justify-between items-center mb-6"><h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center"><ShieldCheck size={14} className="mr-2" />健全度フィルター</h3>{healthFilter !== 'all' && <button onClick={() => setHealthFilter('all')} className="text-[10px] bg-slate-800 text-slate-300 px-2 py-1 rounded hover:bg-slate-700">解除</button>}</div>
          ...
          <div className="p-5 border-b border-slate-800 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-slate-900/50">
            ...
            {isAdding ? (
              <div className="flex items-center space-x-2 bg-slate-800/50 p-1.5 rounded-xl border border-slate-700 animate-in fade-in">
                <input type="text" value={newUsername} onChange={e => setNewUsername(e.target.value)} placeholder="Username" className="text-xs px-3 py-2 outline-none w-32 bg-slate-950 border border-slate-700 rounded-lg text-white" />
                <input type="text" value={newSystemId} onChange={e => setNewSystemId(e.target.value)} placeholder="System ID" className="text-xs px-3 py-2 outline-none w-40 bg-slate-950 border border-slate-700 rounded-lg text-white font-mono" />
            ...
            ) : (<button onClick={() => setIsAdding(true)} className="flex items-center text-xs font-bold text-slate-300 bg-slate-800 hover:bg-slate-700 px-4 py-2 rounded-xl border border-slate-700"><UserPlus size={14} className="mr-2" /> Add Target</button>)}
          </div>
            ...
            <table className="w-full text-left border-collapse min-w-[900px] select-none">
              <thead>
                <tr className="bg-slate-950/50 text-[10px] uppercase tracking-wider font-black text-slate-500 border-b border-slate-800/80">
                  <th className="p-4 pl-6 cursor-pointer hover:text-slate-300 transition-colors" onClick={() => handleSort('username')}>Target <ArrowUpDown size={10} className="inline ml-1" /></th>
                  {/* ★ 各ヘッダーをクリックして並び替え可能に */}
                  <th className="p-4 text-right cursor-pointer hover:text-slate-300 transition-colors" onClick={() => handleSort('total_coins')}>Coins <ArrowUpDown size={10} className="inline ml-1" /></th>
                  <th className="p-4 text-center cursor-pointer hover:text-slate-300 transition-colors" onClick={() => handleSort('unique_listeners')}>Unique <ArrowUpDown size={10} className="inline ml-1" /></th>
                  <th className="p-4 text-center cursor-pointer hover:text-slate-300 transition-colors" onClick={() => handleSort('core_fans')}>Core(1K+) <ArrowUpDown size={10} className="inline ml-1" /></th>
                  <th className="p-4 w-48 cursor-pointer hover:text-slate-300 transition-colors" onClick={() => handleSort('dependency_rate')}>Top1 Dependency <ArrowUpDown size={10} className="inline ml-1" /></th>
                  ...
                  return (
                    <tr key={liver.system_id} onClick={() => setSelectedLiverId(liver.system_id)} className={`cursor-pointer transition-colors group ${isSelected ? 'bg-indigo-500/10 border-l-2 border-indigo-500' : 'hover:bg-slate-800/30 border-l-2 border-transparent'} ${!liver.is_active ? 'opacity-30' : ''}`}>
                      ...
                      <td className="p-4 text-center">
                        {liver.total_coins === 0 ? <span className="text-[10px] font-bold text-slate-600 uppercase border border-slate-700 px-2 py-0.5 rounded">No Data</span>
        ...
        {/* CRM Area (Only shown when selected) */}
        {!selectedLiverId && (
          <div className="bg-slate-900/40 border border-slate-800/50 border-dashed rounded-3xl p-16 flex flex-col items-center justify-center text-slate-500">
             ...
             <p className="font-black tracking-widest uppercase text-lg text-slate-400">Select Target Liver</p>
             <p className="text-sm mt-2 font-medium">上のマトリックスからライバーを選択して、VIP顧客分析を展開してください</p>
          </div>
        )}
                      ...
                      <div key={vip.viewer_id} className={`flex items-center p-3 rounded-xl border transition-colors ${vip.rank === 1 ? 'bg-amber-500/10 border-amber-500/30' : 'bg-slate-950/50 border-slate-800/50 hover:bg-slate-800/80'}`}>
                        ...
                        <div className="flex-grow ml-4 min-w-0">
                          <div className="flex items-center gap-3">
                            ...
                            {vip.total_coins >= 1000 && <span className="text-[10px] font-black tracking-widest text-orange-400 bg-orange-500/10 border border-orange-500/20 px-1.5 py-0.5 rounded uppercase flex items-center"><Flame size={10} className="mr-0.5"/> Core</span>}
                          </div>
