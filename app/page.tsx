...
import { Activity, Coins, Users, Radio, Clock, Calendar, Globe, CalendarSearch } from 'lucide-react';
...
export default function Dashboard() {
  ...
  const displayTotal = activeTab === 'today' ? stats.today 
                     : activeTab === 'month' ? stats.month 
                     : activeTab === 'total' ? stats.total 
                     : stats.custom;
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row gap-4 items-start md:items-center">
          <div className="flex space-x-2 bg-slate-900/50 p-1 rounded-lg border border-slate-800">
            <button onClick={() => setActiveTab('today')} className={`flex items-center px-4 py-2 rounded-md text-sm font-medium transition-all ${activeTab === 'today' ? 'bg-indigo-500 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800'}`}>
              <Clock size={16} className="mr-2" /> 本日
            </button>
            <button onClick={() => setActiveTab('month')} className={`flex items-center px-4 py-2 rounded-md text-sm font-medium transition-all ${activeTab === 'month' ? 'bg-indigo-500 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800'}`}>
              <Calendar size={16} className="mr-2" /> 今月
            </button>
            <button onClick={() => setActiveTab('total')} className={`flex items-center px-4 py-2 rounded-md text-sm font-medium transition-all ${activeTab === 'total' ? 'bg-indigo-500 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800'}`}>
              <Globe size={16} className="mr-2" /> 全期間
            </button>
            <button onClick={() => setActiveTab('custom')} className={`flex items-center px-4 py-2 rounded-md text-sm font-medium transition-all ${activeTab === 'custom' ? 'bg-indigo-500 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800'}`}>
              <CalendarSearch size={16} className="mr-2" /> 期間指定
            </button>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-sm relative overflow-hidden">
            ...
            <p className="text-sm font-medium text-slate-400 mb-2">
              {activeTab === 'today' ? '本日の収益' : activeTab === 'month' ? '今月の収益' : activeTab === 'total' ? '累計収益' : '指定期間の収益'} (ダイヤ)
            </p>
            <h2 className="text-4xl font-extrabold text-white tracking-tight">
              {displayTotal.toLocaleString()}
            </h2>
            <p className="text-xs text-indigo-400 mt-3 flex items-center">
              <Activity size={14} className="mr-1" /> {activeTab === 'custom' ? '過去データ' : 'リアルタイム更新'}
            </p>
          </div>
          ...
          <div className="bg-gradient-to-br from-indigo-600 to-purple-700 p-6 rounded-2xl shadow-lg relative overflow-hidden">
            ...
            <h2 className="text-2xl font-bold text-white tracking-tight mt-1 truncate">pyokotan_54</h2>
