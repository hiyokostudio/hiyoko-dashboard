    ...
    <div className="min-h-screen bg-slate-950 text-slate-50 p-4 md:p-8 font-sans selection:bg-indigo-500/30">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-slate-800 pb-6">
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-white">Hiyoko <span className="text-indigo-400">Analytics</span></h1>
          ...
          <div className="flex items-center space-x-3 bg-slate-900/50 p-2 rounded-xl border border-slate-800">
            ...
            {isAdding ? (
              ...
              <button onClick={() => setIsAdding(true)} className="flex items-center text-xs font-medium text-slate-300 hover:text-white bg-slate-800 hover:bg-slate-700 px-3 py-1.5 rounded-lg transition-colors border border-slate-700">
        ...
        <div className="flex flex-col md:flex-row gap-4 items-start md:items-center">
          ...
          {activeTab === 'custom' && (
            <div className="flex items-center space-x-3 bg-slate-900/50 p-2 rounded-lg border border-slate-800">
              ...
              <button onClick={fetchCustomRange} disabled={!startDate || !endDate || isFetchingCustom} className="bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white px-4 py-1.5 rounded-md text-sm font-medium transition-colors">{isFetchingCustom ? '集計中...' : '集計'}</button>
            </div>
          )}
        </div>
              ...
              {logs.map((log) => (
                <div key={log.id} className="flex items-center justify-between p-3 rounded-lg bg-slate-800/30 hover:bg-slate-800/80 transition-colors border border-slate-700/50">
                  <div className="flex flex-col overflow-hidden">
                    <span className="font-medium text-slate-200 truncate text-sm">{log.viewers?.name || '不明'}</span>
