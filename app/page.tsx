          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4">
            <div className="lg:col-span-2 bg-slate-900/80 border border-slate-800 rounded-3xl p-6 shadow-xl flex flex-col relative overflow-hidden backdrop-blur-md h-[550px]">
              ...
              <div className="flex items-center justify-between mb-6 relative z-10">
                <div className="flex items-center gap-2"><Crown className="h-5 w-5 text-amber-400" /><span className="text-sm font-black text-slate-200">{selectedLiver.liver_name || selectedLiver.username}</span><span className="text-[11px] font-mono text-indigo-400/80 pt-0.5">@{selectedLiver.username}</span><span className="text-sm font-black text-slate-400 ml-1">- 貢献度ランキング (VIP CRM)</span></div>
                <div className="flex items-center gap-2">
              ...
              <div className="flex-grow overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-transparent">
                {vipListeners.length === 0 ? (
                  <div className="flex flex-col items-center justify-center h-full text-slate-600"><Users size={48} className="opacity-20 mb-4" /><p className="font-bold text-sm uppercase tracking-widest">No Listeners</p><p className="text-xs mt-1">指定期間のデータがありません</p></div>
                ) : (
                  <div className="space-y-3">
                        ...
                        <div key={vip.viewer_id} onClick={() => setSelectedViewer({id: vip.viewer_id, name: vip.viewer_name})} className={`flex items-center p-3 rounded-xl border transition-colors cursor-pointer group ${vip.rank === 1 ? 'bg-amber-500/10 border-amber-500/30 hover:bg-amber-500/20' : 'bg-slate-950/50 border-slate-800/50 hover:bg-slate-800/80'}`}>
                          ...
                          <div className="flex-grow ml-4 min-w-0">
                            <div className="flex flex-col relative z-10">
                              <div className="flex items-center gap-2">
                                <span onClick={(e) => { e.preventDefault(); e.stopPropagation(); if (vip.unique_id) window.open(`https://www.tiktok.com/@${vip.unique_id}`, '_blank'); else alert('ID未取得'); }} className={`font-bold text-sm truncate cursor-pointer ${vip.unique_id ? 'hover:underline decoration-slate-400 underline-offset-4' : ''} ${vip.rank === 1 ? 'text-amber-400' : 'text-slate-200'}`}>{vip.viewer_name} {vip.unique_id && <ExternalLink size={10} className="inline text-slate-500 ml-0.5" />}</span>
                              ...
                              <span onClick={(e) => { e.preventDefault(); e.stopPropagation(); if (vip.unique_id) window.open(`https://www.tiktok.com/@${vip.unique_id}`, '_blank'); else alert('ID未取得'); }} className="text-[11px] font-mono font-semibold text-indigo-400/90 truncate cursor-pointer mt-0.5 hover:underline">{vip.unique_id ? `@${vip.unique_id}` : '@ID未取得'}</span>
                            </div>
                            <div className="flex items-center gap-3 mt-1.5">
                              <div className="flex-grow h-1 bg-slate-800 rounded-full overflow-hidden"><div className={`h-full rounded-full transition-all duration-500 ${vip.rank === 1 ? 'bg-amber-400' : vip.total_coins >= 1000 ? 'bg-orange-400' : 'bg-indigo-500'}`} style={{ width: `${contributionRate}%` }}></div></div>
                          ...
                          <div className="flex flex-col items-end ml-4 flex-shrink-0">
                            ...
                            <span className="text-[10px] text-slate-500 font-medium flex items-center mt-1"><Clock size={10} className="mr-1 opacity-50"/> {vip.first_seen ? format(parseISO(vip.first_seen), 'yyyy/MM/dd') : 'データなし'}</span>
                          </div>
                        </div>
                      );
                    })}
                ...
                {detailLogs.map((log) => (
                  <div key={log.id} className="flex items-center justify-between p-3 rounded-xl bg-slate-950/50 hover:bg-slate-800/80 transition-colors border border-slate-800/50">
                    ...
                    <div className="flex items-center gap-3 ml-2 flex-shrink-0">
                      ...
                      <div className="flex items-center bg-emerald-500/10 px-2.5 py-1 rounded-lg border border-emerald-500/20 whitespace-nowrap"><span className="font-black text-emerald-400 text-xs">+{log.coins}</span></div>
                    </div>
                  </div>
                ))}
